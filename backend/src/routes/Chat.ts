...
interface RequestBody {
    message: string;
    userId: string;
    conversationType?: 'general' | 'medical';
}
...
async function fetchFileAsBase64(fileUrl: string): Promise<{ data: string; mimeType: string }> {
    try {
...
async function analyzeUserFilesIfRequested(message: string, userId: string, model: any, chat: any) {
...
// Main message route
router.post('/message', async (req: express.Request<{}, {}, RequestBody>, res: express.Response) => {
    try {
...
router.get('/debug/files/:userId', async (req: express.Request, res: express.Response) => {
    try {
...
router.get('/history/:userId', (req: express.Request, res: express.Response) => {
    const { userId } = req.params;
    const history = conversationHistory.get(userId) || [];

    res.json({
        history: history.map((msg: ConversationMessage) => ({
            role: msg.role,
...
router.delete('/history/:userId', (req: express.Request, res: express.Response) => {
    const { userId } = req.params;
