...
class YouTubeService {
  ...
  private getDiseaseSearchTerms(disease: string): string[] {
  ...
  async searchHealthVideos(disease: string, maxResults: number = 10): Promise<FormattedVideo[]> {
    try {
      ...
      const statsResponse: AxiosResponse<YouTubeVideosResponse> = await axios.get(`${this.baseUrl}/videos`, {
        params: {
          key: this.apiKey,
          id: videoIds,
      ...
      // Format the response
      const formattedVideos: FormattedVideo[] = statsResponse.data.items.map(video => ({
        id: video.id,
